...
- **No prebuilt binary is published** — `build_x64/` isn't committed and there are no [Releases](https://github.com/NoxTaipan/confluence-multistream/releases). You need to build from source: Visual Studio with the "Desktop development with C++" workload + CMake, then run the `Build commands` below (name the build folder `build_x64`, not `build`). No installer can do this step for you.
- Once built, run `install.ps1` (as admin, with OBS closed) — it copies the `.dll` and its data folder into your OBS install. Alternatively, run [confluence-beta](https://github.com/NoxTaipan/confluence-beta)'s `install.bat` and pick "Only Multistream" — it detects your build here and runs this same `install.ps1` for you; see the [suite-wide install guide](https://github.com/NoxTaipan/confluence-beta/blob/master/INSTALL.en.md) for every option.
